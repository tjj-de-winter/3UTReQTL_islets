...
# Compute eQTLs using the R package SCeQTL for a given cell type that is downsampled to a certain cell number.
# Note: this script only performs one repetition, run this script for each downsample iteration!
...
parser.add_argument('--cell_number', type=int,help="New cell number for downsampling")
parser.add_argument('--iteration', help="number of current iteration")
...
parser.add_argument('--group',default='',help='Optional: subgroup or subcondition to use for eQTL calculation')
...
h5ad = args.h5ad
new_cell_number = args.cell_number
...
# downsample to x number of cells
if new_cell_number > len(cells):
    print(f'The dataset contains fewer cells than the numbers of cells to downsample, please try again with less then {len(cells)} cells')
    sys.exit()

cells = random.sample(cells, new_cell_number)

print(name, 'downsampled to', new_cell_number,' cells. Iteration =',iteration) 
...
outfile = '_'.join([outprefix,cell_type, group, iteration, 'eQTL.csv'])
