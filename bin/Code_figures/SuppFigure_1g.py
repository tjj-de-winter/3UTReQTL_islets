...
ct_ND = [ct for ct in cell_types if 'healthy' in ct]

ct_ND_labels = [ct.replace('healthy', 'ND') for ct in cell_types if 'healthy' in ct]
...
combinations = get_combinations(ct_ND)
combinations_labels = get_combinations(ct_ND_labels)

data = []
combinations_list = []
for com, com_label in zip(combinations, combinations_labels):
    count = count_variants(df_eqtl_sig, list(com))
    if count > 0:
        combinations_list.append(com_label)
        data.append(count)
